...
export default function PaymentReturnPageClient({
  ...
  useEffect(() => {
    ...
    if (initialOrderId) {
      fallbackSearchParams.set("orderId", initialOrderId);
    }
    if (initialGuest) {
      fallbackSearchParams.set("guest", initialGuest);
    }

    const context =
      loadAndClearPaymentReturnContext() ??
      loadPaymentReturnContextFromSearchParams(fallbackSearchParams);
    if (!context) {
      setState("no_context");
      return;
    }
  ...
  }, [fetchOrderState, initialGuest, initialOrderId]);
