    ...
    def start(self, context: PaymentStartContext) -> ProviderStartResult:
        ...
        try:
            ...
            try:
                ...
                logger.exception(
                    "AcquireMock request failed: order_id=%s payment_id=%s url=%s timeout=%s body=%s",
                    getattr(context.order, "id", None),
                    getattr(context.payment, "id", None),
                    url,
                    timeout,
                    body,
                )
            ...
            if not response.ok:
                ...
                logger.error(
                    "AcquireMock returned non-2xx: order_id=%s payment_id=%s status=%s body=%s",
                    getattr(context.order, "id", None),
                    getattr(context.payment, "id", None),
            ...
            try:
                ...
                logger.exception(
                    "AcquireMock response parsing failed: order_id=%s payment_id=%s status=%s raw_body=%s",
                    getattr(context.order, "id", None),
                    getattr(context.payment, "id", None),
            ...
            logger.exception(
                "Unexpected AcquireMock provider crash: order_id=%s payment_id=%s",
                getattr(context.order, "id", None),
                getattr(context.payment, "id", None),
            )
            raise
