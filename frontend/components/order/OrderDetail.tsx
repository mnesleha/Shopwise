...
function AddressBlock({
  ...
  const isSupplier = "companyId" in info || "vatId" in info || "bankAccount" in info
    ...
    <Card className="h-full">
      ...
      <CardContent className="flex flex-col gap-2 text-sm">
        ...
        {isSupplier && (supplierInfo.companyId || supplierInfo.vatId) && (
          <div className="mt-2 text-muted-foreground">
            {supplierInfo.companyId && <p>Company ID: {supplierInfo.companyId}</p>}
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-right text-muted-foreground">${item.unitPrice}</td>
                  <td className="px-4 py-3 text-right text-muted-foreground">
                    {formatDiscount(item.discount)}
                  </td>
                  <td className="px-4 py-3 text-right text-foreground font-medium">${item.lineTotal}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function TotalsSummary({ order }: { order: OrderViewModel }) {
  ...
  const breakdownRows: MoneyLine[] = order.totalsBreakdown ?? [
    ...
    ...(order.shippingFee ? [{ label: "Shipping", amount: order.shippingFee }] : []),
    ...(order.discountTotal ? [{ label: "Discount", amount: `-${order.discountTotal}` }] : []),
  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-2">
        ...
        {!order.taxTotal && (
          <p className="text-xs text-muted-foreground italic">Tax included in price</p>
        )}
  ...
  return (
    <div className="mx-auto max-w-4xl print:max-w-none">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        <Button variant="outline" onClick={onBackToShop} className="gap-2 bg-transparent">
          <ArrowLeft className="h-4 w-4" />
          Back to shop
        </Button>
        <div className="flex gap-2">
          {onPrint && (
            <Button variant="outline" onClick={onPrint} className="gap-2 bg-transparent">
              <Printer className="h-4 w-4" />
              Print
            </Button>
          )}
          {onDownloadPdf && (
            <Button variant="outline" onClick={onDownloadPdf} className="gap-2 bg-transparent">
      ...
      <div className="rounded-lg border border-border bg-background p-6 shadow-sm print:border-0 print:shadow-none print:p-0">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <AddressBlock title="Supplier" info={order.supplier} showBankAccount />
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm">
          {order.createdAt && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-foreground font-medium">{order.createdAt}</span>
            </div>
          )}
          <div className="flex items-center gap-2">
          ...
          {order.shippingMethod && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-foreground font-medium">{order.shippingMethod}</span>
            </div>
          )}
          {order.paymentMethod && (
            <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-2 text-center text-sm text-muted-foreground">
          ...
          <p>
            ...
            <a href="mailto:support@shopwise.demo" className="text-primary hover:underline">
...
export type { OrderViewModel, SupplierInfo, CustomerInfo, OrderItem, MoneyLine }
