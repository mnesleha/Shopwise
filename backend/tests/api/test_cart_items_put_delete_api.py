"""
Tests for cart item quantity update via PATCH /api/v1/cart/items/{product_id}/
and for item removal via DELETE /api/v1/cart/items/{product_id}/.

Previously the update endpoint used PUT; it now uses PATCH (partial update semantics).
Business behavior is unchanged — only the HTTP method differs.
"""
...
def test_patch_cart_item_creates_item_returns_201_and_cart_snapshot(auth_client):
...
@pytest.mark.django_db
def test_patch_cart_item_updates_existing_item_returns_200_and_cart_snapshot(auth_client):
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)
...
@pytest.mark.django_db
def test_patch_cart_item_quantity_zero_removes_item_returns_200_cart_snapshot(auth_client):
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)
...
@pytest.mark.django_db
def test_patch_cart_item_quantity_negative_returns_400(auth_client):
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)
...
@pytest.mark.django_db
def test_patch_cart_item_quantity_missing_returns_400(auth_client):
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)
...
@pytest.mark.django_db
def test_patch_cart_item_out_of_stock_returns_409(auth_client):
    product = Product.objects.create(
...
# ---------------------------------------------------------------------------
# PUT is no longer a supported write method — expect 405
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_put_cart_item_returns_405_method_not_allowed(auth_client):
    """PUT has been replaced by PATCH; the endpoint must reject PUT with 405."""
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)
...
@pytest.mark.django_db
def test_guest_patch_creates_sets_cookie_and_updates_and_delete_works():
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)
    client = APIClient()
...
@pytest.mark.django_db
def test_guest_can_use_cart_token_header_for_patch_and_delete():
    product = Product.objects.create(
        name="P", price=10, stock_quantity=10, is_active=True)

    c1 = APIClient()
    r1 = c1.patch(f"/api/v1/cart/items/{product.id}/",
    ...
    r2 = c2.patch(f"/api/v1/cart/items/{product.id}/",
...
@pytest.mark.django_db
def test_patch_inactive_product_returns_error(auth_client):
    product = Product.objects.create(
